    ...
    $.ajax({
      type: 'POST',
      url: '../production/tabla_actividades.php',
      data: {'estado': estado, 'user': user, 'id_user': id_user}
    })
    .done(function(obtenerDatos){
      $('#div_tabla_actividades').html(obtenerDatos);
...
$('input[id=estado_a]').on('change', function() {
  if ($(this).is(':checked') ) {
    var estado = "Activo";
    var user = $('#user').val();
    var id_user = $('#id_usuario').val();
...
function mostrar_actividad(id){
    $("#mostrar").val(id);
    $("#from_mostrar_actividad").submit();
   }
   ...
   function dar_baja_actividad(id){
   ...
   function dar_alta_actividad(id){
