                    ...
                    <table id="datatable-responsive" class="table table-striped table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
                      ...
                      <tbody>
                        ...
                        if($user=="Administrador General"){
                          $stmt= $pdo->prepare("SELECT c.id_cooperante, c.nombre_cooperante, c.monto, c.tipo_ayuda, DATE_FORMAT(c.fecha_ingreso, '%d/%m/%Y') AS fecha_ingreso, e.nombre, e.apellido FROM cooperante AS c INNER JOIN emprendedor AS e ON (c.id_emprendedor=e.id_emprendedor) WHERE c.id_emprendedor=:id_emprendedor");
                          $stmt->bindParam(":id_emprendedor",$id_emprendedor,PDO::PARAM_STR);
                          $stmt->execute();
                        }else if($user=="Administrador Territorio"){
                          $stmt= $pdo->prepare("SELECT c.id_cooperante, c.nombre_cooperante, c.monto, c.tipo_ayuda, DATE_FORMAT(c.fecha_ingreso, '%d/%m/%Y') AS fecha_ingreso, e.nombre, e.apellido FROM cooperante AS c INNER JOIN emprendedor AS e ON (c.id_emprendedor=e.id_emprendedor) WHERE c.id_usuario=:id_user AND c.id_emprendedor=:id_emprendedor");
